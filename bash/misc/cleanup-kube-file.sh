...
referenced_clusters=$(kubectl $CONFIG_FILE config view -o jsonpath='{.contexts[*].context.cluster}' | tr ' ' '\n' | sort | uniq)
referenced_users=$(kubectl $CONFIG_FILE config view -o jsonpath='{.contexts[*].context.user}' | tr ' ' '\n' | sort | uniq)
...
all_clusters=$(kubectl $CONFIG_FILE config view -o jsonpath='{.clusters[*].name}' | tr ' ' '\n' | sort | uniq)
all_users=$(kubectl $CONFIG_FILE config view -o jsonpath='{.users[*].name}' | tr ' ' '\n' | sort | uniq)
...
for cluster in $all_clusters; do
    if ! echo "$referenced_clusters" | grep -qx "$cluster"; then
        ...
        kubectl $CONFIG_FILE config delete-cluster "$cluster"
    fi
done
...
for user in $all_users; do
    if ! echo "$referenced_users" | grep -qx "$user"; then
        ...
        kubectl $CONFIG_FILE config delete-user "$user"
    fi
done
