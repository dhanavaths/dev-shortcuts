...
function _fetch_object() {
  ...
  object_name=$1
  echo "kubectl $kube_context $kube_config $OPT_NAMESPACE $resource_verb $resource_kind $object_name $var_arguments ${OPT_OUTPUT_FORMAT}" >&2
  if [ "${OPT_LESS_MODE}" = "less" ]; then
    kubectl $kube_context $kube_config $OPT_NAMESPACE $resource_verb $resource_kind $object_name $var_arguments ${OPT_OUTPUT_FORMAT} | less
  elif [ "${OPT_VSCODE_MODE}" = "code" ]; then
    kubectl $kube_context $kube_config $OPT_NAMESPACE $resource_verb $resource_kind $object_name $var_arguments ${OPT_OUTPUT_FORMAT} | code -
  else
    kubectl $kube_context $kube_config $OPT_NAMESPACE $resource_verb $resource_kind $object_name $var_arguments ${OPT_OUTPUT_FORMAT}
  fi
  echo
}
...
function _main() {
  if [ -n "${OPT_ALL_NAMESPACES}" ]; then
    kubectl $kube_context $kube_config get $resource_kind ${OPT_ALL_NAMESPACES} ${OPT_LABEL_SELECTOR} -owide
    if [ "${OPT_OUTPUT_FORMAT}" == "-oyaml" ]; then
      kubectl $kube_context $kube_config $OPT_NAMESPACE get $resource_kind ${OPT_ALL_NAMESPACES} ${OPT_LABEL_SELECTOR} -oyaml | code -
    fi
    return
      ...
      if [[ -z "${OPT_OUTPUT_FORMAT}" ]] || [[ "${OPT_OUTPUT_FORMAT}" == "-owide" ]]; then
        echo "kubectl $kube_context $kube_config $OPT_NAMESPACE get $resource_kind ${OPT_RESOURCE_NAME} ${OPT_SORT_BY} -owide" >&2
        kubectl $kube_context $kube_config $OPT_NAMESPACE get $resource_kind ${OPT_RESOURCE_NAME} ${OPT_SORT_BY} -owide
      fi
    ...
    echo "$_output"
    resource_list=$(echo "$_output" | awk 'NR > 1 {print $1}' | nl -v 0)
    # resource_list=$(kubectl $kube_context $kube_config $OPT_NAMESPACE get $resource_kind ${OPT_NODE_NAME_SELECTOR} -o jsonpath='{.items[*].metadata.name}' | tr ' ' '\n' | nl -v 0)
  fi
